# -*- coding: utf-8 -*- # Přidáno pro jistotu (i když Python 3 by měl být OK)

import tkinter as tk
from tkinter import ttk
from tkinter import filedialog, messagebox, simpledialog # Přidán simpledialog
import csv
import os
import sys
import re
import shutil # Pro přepsání souboru
...
cary_filepath = None
main_root = None # Reference na hlavní okno
status_label = None # Reference na stavový řádek
...
def parse_connection(connection_string):
    ...
    # Vracíme None, pokud parsování některé části selhalo (vrátilo None pro name/pin)
    if not comp_a or not comp_b or comp_a['pin'] is None or comp_b['pin'] is None:
         # print(f"Debug parse_connection: Selhalo parsování části pro '{connection_string}' -> A:{comp_a}, B:{comp_b}")
         return None
...
def read_encoding(filename=ENCODING_FILE):
    """Načte znakovou sadu."""
    global status_label # Potřebujeme pro případnou chybu
    try:
...
def write_output_file(output_filepath, encoding, header, data, rows_processed, rows_written, success_status_msg):
    ...
    output_filename = os.path.basename(output_filepath)
    final_status = success_status_msg # Uložíme si původní status
    ...
    else: status_label.config(text=f"Zapisuji {rows_written} řádků do {output_filename}...")
    try:
        ...
        if final_status and status_label: status_label.config(text=final_status) # Aktualizujeme status až po úspěšném zápisu
...
def process_texty_csv(input_filepath, encoding):
    ...
    try:
        ...
        if not check_overwrite(output_filepath): status_label.config(text=f"Zpracování {output_filename} zrušeno."); return False
...
def merge_and_deduplicate(cary_upr_path, symboly_upr_path, texty_upr_path, output_path, encoding):
    ...
    global status_label # Přístup ke GUI
    status_label.config(text=f"Slučuji data -> {os.path.basename(output_path)}")
    print(f"\n--- Slučování a deduplikace ---"); print(f"-> {output_path}")
    unique_connections = {}; processed_count = 0; skipped_no_slash = 0
    def get_canonical(connection_string):
        parsed = parse_connection(connection_string)
        if not parsed: return None, None, None # Nelze parsovat nebo chybí /
    ...
    status_label.config(text=f"Zapisuji {len(unique_connections)} spojení -> {os.path.basename(output_path)}")
    try: # Zápis
        with open(output_path, 'w', encoding=encoding, newline='') as outfile:
            ...
            # Seřadíme výstup podle klíče (spojení) pro konzistentní pořadí
            for connection in sorted(unique_connections.keys()):
    ...
    try: # Kontrola duplicit
        status_label.config(text=f"Kontrola duplicit v {os.path.basename(output_path)}...")
...
def select_file(file_type_var, label_widget, suffix_filter):
    ...
    global texty_filepath, symboly_filepath, cary_filepath, status_label # Přidáno status_label

    # Získání cesty k adresáři skriptu jako výchozí
    try:
        initial_dir = os.path.dirname(os.path.abspath(sys.argv[0]))
    except Exception:
        initial_dir = os.getcwd() # Fallback na aktuální pracovní adresář

    filetypes = ((f"*{suffix_filter}", f"*{suffix_filter}"), ("CSV soubory", "*.csv"), ("Všechny soubory", "*.*"))

    filepath = filedialog.askopenfilename(
        initialdir=initial_dir,
        title=f"Vyberte soubor končící na {suffix_filter}",
        filetypes=filetypes
    )

    if filepath:
        # Ověření koncovky souboru
        if not filepath.lower().endswith(suffix_filter.lower()):
             messagebox.showwarning("Nesprávný typ souboru", f"Vybraný soubor '{os.path.basename(filepath)}' nekončí na '{suffix_filter}'.\nVyberte prosím správný soubor.")
             return # Neaktualizujeme cestu ani label

        filename = os.path.basename(filepath)
        # Aktualizace globální proměnné a popisku v GUI
        if file_type_var == 'texty':
            texty_filepath = filepath
            label_widget.config(text=f"{filename}") # Zkráceno jen na název souboru
        elif file_type_var == 'symboly':
            symboly_filepath = filepath
            label_widget.config(text=f"{filename}") # Zkráceno jen na název souboru
        elif file_type_var == 'cary':
            cary_filepath = filepath
            label_widget.config(text=f"{filename}") # Zkráceno jen na název souboru

        if status_label: # Zkontrolujeme, zda status_label existuje
            status_label.config(text=f"Soubor '{filename}' vybrán.")
    else:
         if status_label: # Zkontrolujeme, zda status_label existuje
            status_label.config(text="Výběr souboru zrušen.")
# --- Třída Editoru ---
from collections import Counter # Použijeme pro snadné počítání

class SpecEditorApp:
    def __init__(self, parent, input_file, output_file, encoding):
        # ... (začátek __init__ zůstává stejný) ...
        ...
        self.component_specs = defaultdict(list) # Nový slovník: {'PUC1': ['spec1', 'spec2'], ...}
        self.suggested_specs = {} # Nový slovník: {'PUC1': 'nejcastejsi_spec', ...}
    ...
    def on_close(self):
        ...
        print("- Editor zavřen.")
        # Možná zde přidat dotaz, zda chce uživatel uložit neuložené změny
        self.editor_window.destroy()

       # !!!!! UPRAVENÁ METODA load_data !!!!!
    def load_data(self):
        """Načte data z priprava.csv, naparsuje je a analyzuje specifikace."""
        global status_label
        status_label.config(text=f"Načítám a analyzuji data pro editor...")
        print(f"+ Načítám editor: {self.input_file}")
        component_names_a = set(); component_names_b = set()
        self.component_specs.clear() # Vyčistit předchozí data
        ...
        try:
            with open(self.input_file, 'r', encoding=self.encoding, newline='') as f:
                ...
                for i, row in enumerate(reader):
                    ...
                    is_default_spec = current_spec_lower == DEFAULT_SPECIFICATION.lower() or not spec

                    if is_default_spec:
                        self.items_to_edit_ids.add(item_id) # Označit k editaci

                    # Sbíráme informace pro групиování a návrhy
                    if parsed_data:
                        ...
                        # Přidáme názvy do množin pro групиování
                        if comp_a_name: component_names_a.add(comp_a_name)
                        if comp_b_name: component_names_b.add(comp_b_name)

                        # Pokud má řádek platnou specifikaci, přidáme ji k dané komponentě
            ...
            # --- Analýza a příprava návrhů ---
            self.suggested_specs.clear()
            print("+ Analyzuji nejčastější specifikace pro komponenty...")
            for comp_name, spec_list in self.component_specs.items():
                if spec_list:
                    # Najdeme nejčastější specifikaci pomocí Counter
                    spec_counts = Counter(spec_list)
                    most_common_spec, count = spec_counts.most_common(1)[0]
                    # Můžeme přidat podmínku na minimální počet výskytů, aby návrh byl relevantní
                    if count > 1: # Např. navrhujeme, jen pokud se spec vyskytla víckrát než jednou
                        self.suggested_specs[comp_name] = most_common_spec
                        # print(f"  Návrh pro '{comp_name}': '{most_common_spec}' (nalezeno {count}x)")

            # --- Příprava klíčů pro Combobox ---
            self.group_keys.clear()
            self.group_keys["Všechny 'neni spec'"] = lambda item: item['id'] in self.items_to_edit_ids

            # Skupiny podle názvu A (s návrhem, pokud existuje)
            for name in sorted(list(component_names_a)):
                suggestion = self.suggested_specs.get(name)
                key = f"Název A: {name}"
                if suggestion:
                    key += f" (návrh: '{suggestion}')" # Přidáme návrh do klíče
                # Filtr zůstává stejný (podle jména), návrh je jen v textu klíče
                self.group_keys[key] = lambda item, n=name: item['id'] in self.items_to_edit_ids and item['parsed'] and item['parsed']['a']['name'] == n

            # Skupiny podle názvu B (s návrhem, pokud existuje)
            for name in sorted(list(component_names_b)):
                suggestion = self.suggested_specs.get(name)
                key_base = f"Název B: {name}"
                key = key_base
                if suggestion:
                    key += f" (návrh: '{suggestion}')"
                # Ošetření případné kolize klíčů (pokud A i B mají stejný název)
                if key in self.group_keys:
                     # Pokud už existuje klíč s návrhem, vytvoříme klíč bez návrhu pro B
                     if f" (návrh: '" in key:
                          key = key_base + " (B)"
                          if key in self.group_keys: # Opravdu vzácná kolize
                              key += "_2"
                     else: # Pokud existuje klíč bez návrhu, přidáme (B)
                          key += " (B)"
                self.group_keys[key] = lambda item, n=name: item['id'] in self.items_to_edit_ids and item['parsed'] and item['parsed']['b']['name'] == n


            print(f"+ Editor data: {len(self.all_data)} řádků, {len(self.items_to_edit_ids)} k editaci. Připraveno {len(self.suggested_specs)} návrhů.")
            status_label.config(text=f"Editor načten: {len(self.items_to_edit_ids)} položek k editaci.")
    ...
    def create_widgets(self):
        ...
        # ... (začátek create_widgets zůstává stejný) ...
        ...
        # Zvětšíme šířku Comboboxu, aby se vešly návrhy
        self.group_combo = ttk.Combobox(top_frame, values=list(self.group_keys.keys()), state="readonly", width=60)
        self.group_combo.grid(row=0, column=1, padx=(0, 20), sticky=tk.W)
        ...
        # ... (zbytek create_widgets - Treeview, tlačítka dole - zůstává stejný) ...
    ...
    # !!!!! UPRAVENÁ METODA on_group_select !!!!!
    def on_group_select(self, event=None):
        ...
        filter_func = self.group_keys.get(filter_key)

        # Vyčistit pole pro specifikaci
        self.spec_entry.delete(0, tk.END)

        # Zkusit extrahovat návrh z klíče
        match = re.search(r"\(návrh: '(.*)'\)", filter_key)
        if match:
            suggested_spec = match.group(1)
            self.spec_entry.insert(0, suggested_spec) # Předvyplnit pole
            print(f"+ Předvyplněn návrh: '{suggested_spec}' pro skupinu '{filter_key}'")

        # Aplikovat filtr na Treeview
        if filter_func:
            self.populate_treeview(filter_func)
        else:
            print(f"! Chyba: Nenalezena funkce filtru pro '{filter_key}'")

    def apply_to_group(self):
        """Aplikuje specifikaci z Entry na všechny viditelné řádky v Treeview,
           ale nechá je viditelné."""
        # ... (zůstává stejné jako v předchozí verzi) ...
        ...
        for item_id in visible_item_ids:
            for item_data in self.all_data:
                if item_data['id'] == item_id:
                    ...
                    if item_id in self.items_to_edit_ids: self.items_to_edit_ids.remove(item_id) # Označit jako upravené
                    try: self.tree.item(item_id, values=(item_data['spojeni'], new_spec))
                    except tk.TclError: print(f"! Varování: Nepodařilo se aktualizovat Treeview pro {item_id}.")
                    updated_count += 1; break
        print(f"+ Specifikace '{new_spec}' aplikována na {updated_count} položek.")
        ...
        # self.spec_entry.delete(0, tk.END) # Necháme text v poli pro případné další použití

    def edit_selected_item_popup(self, event):
    ...
    def save_changes(self):
        ...
        try:
            ...
            status_label.config(text=f"Změny uloženy do {output_basename}.")
            self.on_close() # Zavřít okno
...
def run_filter_and_merge():
    ...
    if not texty_filepath: messagebox.showerror("Chybí", f"Nevybrán soubor '{TEXTY_SUFFIX}'."); return
    if not symboly_filepath: messagebox.showerror("Chybí", f"Nevybrán soubor '{SYMBOLY_SUFFIX}'."); return
    if not cary_filepath: messagebox.showerror("Chybí", f"Nevybrán soubor '{CARY_SUFFIX}'."); return
    file_encoding = read_encoding(ENCODING_FILE)
    if file_encoding is None: status_label.config(text="Chyba kódování."); return
    status_label.config(text="Fáze 1: Zpracování vstupů...")
    ...
    status_label.config(text="Fáze 1 OK. Zahajuji Fázi 2: Slučování...")
    ...
    if not os.path.exists(texty_upr_path) or not os.path.exists(symboly_upr_path) or not os.path.exists(cary_upr_path):
         messagebox.showerror("Chyba", f"Některý z '_upr.csv' souborů nebyl nalezen pro sloučení."); return
...
def run_editor():
    ...
    if not os.path.exists(priprava_csv_path):
        messagebox.showerror("Chyba", f"Soubor '{FINAL_OUTPUT_FILENAME}' nenalezen.\nNejprve spusťte 'Zpracovat a Sloučit'.")
        status_label.config(text=f"Soubor {FINAL_OUTPUT_FILENAME} nenalezen.")
        return
    file_encoding = read_encoding(ENCODING_FILE)
    if file_encoding is None: status_label.config(text="Chyba kódování pro editor."); return
    status_label.config(text="Spouštím editor specifikací...")
    editor = SpecEditorApp(main_root, priprava_csv_path, edited_output_path, file_encoding)
...
def run_overwrite_original():
    ...
    if not os.path.exists(edited_output_path):
        messagebox.showerror("Chyba", f"Soubor '{EDITED_OUTPUT_FILENAME}' nenalezen.\nSpusťte editor a uložte změny.")
        status_label.config(text=f"Soubor {EDITED_OUTPUT_FILENAME} nenalezen.")
        return
    ...
    if not confirm: status_label.config(text="Přepsání originálu zrušeno."); return
    try:
        print(f"+ Přesouvám '{edited_output_path}' -> '{priprava_csv_path}'")
        shutil.move(edited_output_path, priprava_csv_path) # Přesune a přepíše cíl
        messagebox.showinfo("Hotovo", f"Soubor '{FINAL_OUTPUT_FILENAME}' byl úspěšně přepsán.")
        status_label.config(text=f"Soubor {FINAL_OUTPUT_FILENAME} přepsán.")
...
if __name__ == "__main__":
    main_root = tk.Tk()
    main_root.title("CSV Nástroje v1.0") # Verze
